...
class Vehicle(RoadObject):
    ...
    WIDTH = 2.0
    """ Vehicle width [m] """
    DEFAULT_SPEEDS_LIMIT = [0.7, 0.8]  # add
    DEFAULT_SPEEDS = [23, 25]  # dns_debug
    """ Range for random initial speeds [m/s] """
    """ If there is lane_speed_limit, from 0.7 to 0.8"""
    MAX_SPEED = 40.
    ...
    def create_random(cls, road: Road,
                      ...
                      spacing: float = 1,
                      init_speed: list = DEFAULT_SPEEDS_LIMIT) \
            -> "Vehicle":
        """
        ...
        :param init_speed: initial init_speed
        :param road: the road where the vehicle is driving
        ...
        """
        Vehicle.DEFAULT_SPEEDS_LIMIT = init_speed
        ...
        if speed is None:
            if lane.speed_limit is not None:
                ...
                speed = road.np_random.uniform(Vehicle.DEFAULT_SPEEDS_LIMIT[0]*lane.speed_limit,
                                               Vehicle.DEFAULT_SPEEDS_LIMIT[1]*lane.speed_limit)
            else:
        ...
        """Reset the DEFAULT_SPEEDS_LIMIT"""
    ...
    def check_out_of_lane(self) -> None:  # dns_function
        """
        find the distance to nearest line

        :return:
        """
        if not self.crashed:
            if self.lane.distance_with_heading(self.position, self.heading) > 3:
                self.crashed = True
            else:
                if not self.on_road:
                    self.crashed = True
    ...
    def to_dict_dns(self, origin_vehicle: "Vehicle" = None, observe_intentions: bool = True) -> dict: # dns_add
        d = {
            'presence': 1,
            'x': self.position[0],
            'y': self.position[1],
    ...
    def to_dict_dns2(self, origin_vehicle: "Vehicle" = None, observe_intentions: bool = True) -> dict: # dns_add
        d = {
            'presence': 1,
            'x': self.position[0],
            'y': self.position[1],
