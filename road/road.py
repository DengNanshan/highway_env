...
class Road(object):
    ...
    def close_vehicles_to(self, vehicle: 'kinematics.Vehicle', distance: float, count: int = None,
        ...
        # vehicles = sorted(vehicles, key=lambda v: (vehicle.lane_distance_to(v))) # dns
        vehicles = sorted(vehicles, key=lambda v: abs(vehicle.lane_distance_to(v)))  # dns
        return vehicles

    def around_vehicles_to(self, vehicle: 'kinematics.Vehicle', distance: float, see_behind: bool = True,
                           lane_number: int = 4) -> object:  # dns_func

        lane_index = self.vehicles[0].lane_index
        lane=lane_index[2]
        f, r = self.neighbour_vehicles(vehicle=self.vehicles[0])
        if lane == 0:
            fl, rl = None, None
        else:
            fl, rl = self.neighbour_vehicles(vehicle=self.vehicles[0],lane_index=('0', '1', lane - 1))

        if lane == lane_number-1:   # todo how to get max lane_index
            fr, rr = None, None
        else:
            fr, rr = self.neighbour_vehicles(vehicle=self.vehicles[0], lane_index=('0', '1', lane + 1))

        vehicles = [f, r, fl, rl, fr, rr]
        return vehicles

    def around_vehicles_to2(self, vehicle: 'kinematics.Vehicle', distance: float, see_behind: bool = True,
                           lane_number: int = 4) -> object:  # dns_func
    ...
    def step(self, dt: float) -> None:
        ...
        :param dt: timestep [s]
        """
        self.vehicles[0].check_out_of_lane()  # dns_debug
        for vehicle in self.vehicles:
            vehicle.step(dt)
