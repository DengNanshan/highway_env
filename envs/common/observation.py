...
class KinematicObservation(ObservationType):
    ...
    def observe(self) -> np.ndarray:
        ...
        around_vehicles = self.env.road.around_vehicles_to(self.observer_vehicle, self.env.PERCEPTION_DISTANCE,
                                                           lane_number=self.env.config["lanes_count"])  # dns_add

        # if close_vehicles:
        if around_vehicles:  # dns_debug
            origin = self.observer_vehicle if not self.absolute else None
            for i, v in enumerate (around_vehicles): # [f, r, fl, rl, fr, rr]
                if v is None:
                    if i==1 or i==3 or i == 5:  # f
                        df=df.append({'presence': 0, 'x': -1000, 'y': 1000, 'vx': -100, 'vy': 0, 'heading': 0.0, 'cos_h': 1.0, 'sin_h': 0.0, 'cos_d': 0, 'sin_d': 0},['0'])
                    else:
                        df = df.append(
                            {'presence': 0, 'x': 1e6, 'y': 1000, 'vx': -100, 'vy': 0, 'heading': 0.0, 'cos_h': 1.0,
                             'sin_h': 0.0, 'cos_d': 0, 'sin_d': 0}, ['0'])
                else:
                    df = df.append(v.to_dict_dns(origin, observe_intentions=self.observe_intentions), ['0'])

        # # dns_delate # dns_add change the order from nest
        # if close_vehicles:
        #     origin = self.observer_vehicle if not self.absolute else None
        #     df = df.append(pd.DataFrame.from_records(
        #         [v.to_dict_dns(origin, observe_intentions=self.observe_intentions)
